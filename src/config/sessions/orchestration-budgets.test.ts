...
describe("session orchestration budgets", () => {
  ...
  it("admits work only while the durable shared counter has capacity", async () => {
    await seedOwner();
    await createSessionOrchestrationBudget({
      ownerSessionKey,
      storePath,
    ...
    await chargeSessionOrchestrationBudget({
      ownerSessionKey,
      storePath,
      rootRunId: "run-root",
      tokens: 10,
    ...
    ).toThrow("orchestration token budget exhausted (10/10)");
  });
});
