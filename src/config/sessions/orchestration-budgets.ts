...
/** Fails closed when a referenced shared budget cannot admit another paid action. */
export function assertSessionOrchestrationBudgetAvailable(
  scope: OrchestrationBudgetScope & { rootRunId: string },
): SessionOrchestrationBudget {
  const budget = getSessionOrchestrationBudget(scope);
  if (!budget) {
    throw new Error("orchestration budget not found");
  }
  if (budget.rootRunId !== scope.rootRunId.trim()) {
    throw new Error("orchestration budget root run mismatch");
  }
  if (budget.tokensUsed >= budget.tokenLimit) {
    throw new Error(
      `orchestration token budget exhausted (${budget.tokensUsed}/${budget.tokenLimit})`,
    );
  }
  return budget;
}
