...
it("rejects receipts bound to a changed provider contract version", () => {
  expect(() =>
    parseContinuityPublicationAcceptanceReceiptV1({
      ...
      publicationPluginId: "example-host",
      publicationBindingId: "example/continuity",
      publicationBindingVersion: "continuity-publication-provider/v2",
      publicationBindingGeneration: "binding-7",
    }),
  ).toThrow(/publicationBindingVersion/u);
});
...
function registerProvider(params: {
  ...
  const pluginId = params.pluginId ?? "example-host";
...
describe("continuity publication provider", () => {
  it("rejects malformed registrations and retains duplicates to fail closed", () => {
    ...
    expect(registry.registry.continuityPublicationProviders).toHaveLength(2);
  ...
  it("rejects a provider whose runtime registration differs from the manifest owner", async () => {
    const registry = createRegistry();
    const reference = registerProvider({
      registry,
      pluginId: "other-host",
    ...
    await expect(
      publishContinuityArtifactV1({
        registry: registry.registry,
        reference: { ...reference, pluginId: "example-host" },
        identity,
        content: chunkedContent(),
        signal: new AbortController().signal,
      }),
  ...
  it("returns the original acceptance on exact replay and conflicts on changed content", async () => {
    ...
    const changedContent = new TextEncoder().encode("changed continuity fixture");
    await expect(
      publishContinuityArtifactV1({
        ...request,
        identity: {
          ...identity,
          archiveSha256: createHash("sha256").update(changedContent).digest("hex"),
          archiveSize: changedContent.byteLength,
        },
        content: (async function* () {
          yield changedContent;
        })(),
      }),
    ).rejects.toMatchObject({ code: "conflict" });
  });

  it("fails closed on stale provider generation and changed plugin ownership", async () => {
    const registry = createRegistry();
    const reference = registerProvider({
      registry,
      provider: createFakeProvider(new Map(), []),
    ...
    await expect(
      publishContinuityArtifactV1({
        registry: registry.registry,
        reference: { ...reference, pluginId: "replacement-host" },
        identity,
        content: chunkedContent(),
        signal: new AbortController().signal,
      }),
  ...
  it("rejects retrieval for another trusted owner", async () => {
    const registry = createRegistry();
    const reference = registerProvider({
      registry,
      provider: createFakeProvider(new Map(), []),
    ...
    await expect(
      retrieveContinuityArtifactV1({
        ...
        reference,
        receipt,
        expectedOwnerId: "tenant/cell-2",
        signal: new AbortController().signal,
      }),
    ).rejects.toMatchObject({ code: "invalid-request" });
  });

  it("rejects acceptance when the provider does not consume the complete stream", async () => {
    const registry = createRegistry();
    const provider = createFakeProvider(new Map(), []);
    provider.publish = async ({ identity: requested }) => ({
      version: CONTINUITY_PUBLICATION_ACCEPTANCE_VERSION,
      publicationId: `publication/${requested.handoffId}`,
      identity: requested,
      durabilityClass: "immutable",
