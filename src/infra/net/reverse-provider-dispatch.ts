...
function requireHeaderRecord(value: unknown, label: string): Record<string, string> {
  ...
  for (const [key, entry] of Object.entries(record)) {
    ...
    if (normalizedNames.has(normalized)) {
      throw new Error(`${label} contains duplicate case-insensitive header names`);
    }
...
function assertFrameWireLimit(record: Record<string, unknown>): void {
  if (encodedFrameBytes(record) > REVERSE_PROVIDER_DISPATCH_MAX_FRAME_BYTES_V1) {
    throw new Error("frame exceeds the reverse provider dispatch wire limit");
  }
}
...
function assertOperationOpen(record: Record<string, unknown>): void {
  ...
  requirePositiveInteger(record, "timeoutMs", "operation-open");
  ...
  if (maxFrameBytes > REVERSE_PROVIDER_DISPATCH_MAX_FRAME_BYTES_V1) {
    throw new Error("operation-open.maxFrameBytes exceeds the wire limit");
  }
  if (
