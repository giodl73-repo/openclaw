...
const RECEIPT_STORE_BUSY_TIMEOUT_MS = 5_000;
...
function rowToRecordedReceipt(row: ReceiptRow): RecordedAuditReceipt {
  ...
  return {
    ...
    ...(row.receipt_version === null ? {} : { receiptVersion: row.receipt_version }),
...
function receiptDataJson(receipt: AgentToolAuditRecord): string | null {
  if (!receipt.data) {
    return null;
  }
  const json = JSON.stringify(receipt.data);
...
function buildFilteredReceiptQuery(
  ...
  if (filters.receiptType) {
    query = query.where("receipt_type", "=", filters.receiptType);
  }
  if (filters.agentIds && filters.agentIds.length > 0) {
    query = query.where("agent_id", "in", filters.agentIds);
  }
...
/** Lists newest matching receipts from the shared cross-agent store. */
export function listAuditReceipts(params: {
  ...
  const rows = executeSqliteQuerySync(
    database.db,
    buildFilteredReceiptQuery(db, params.filters ?? {})
      .selectAll()
      .orderBy("sequence", "desc")
      .limit(Math.max(0, Math.floor(params.limit))),
  ).rows;
  return rows.map(rowToRecordedReceipt);
}
