...
describe("shared audit receipt store", () => {
  ...
  it("stores and filters outcomes shared by multiple agents", () => {
    ...
    expect(
      listAuditReceipts({
        filters: { receiptType: "payment.authorized" },
        limit: 10,
  ...
  it("deduplicates replay of the same trusted source", () => {
    const first = record({ agentId: "support", receiptType: "case.resolved" });
    const replay = record({ agentId: "support", receiptType: "case.resolved" });
