...
function shellRestartCommand(shell: CompletionShell): string {
  return `source ~/.${shell === "zsh" ? "zshrc" : shell === "bash" ? "bashrc" : shell === "fish" ? "config/fish/config.fish" : "config/powershell/Microsoft.PowerShell_profile.ps1"}`;
}
...
export async function repairShellCompletionHealth(params: {
  ...
  if (status.usesSlowPattern) {
    ...
    changes.push(
      `Shell completion upgraded. Restart your shell or run: ${shellRestartCommand(status.shell)}`,
    );
  ...
  if (!status.profileInstalled) {
    ...
    await install(status.shell, true, cliName);
    changes.push(
      `Shell completion installed. Restart your shell or run: ${shellRestartCommand(status.shell)}`,
    );
  }
