...
export type ProviderRequestTrafficPolicyProcessStateV1 =
  | { owner: string; snapshot: ProviderRequestTrafficPolicySnapshotV1; active: boolean }
  | undefined;

const ID_RE = /^[a-z0-9][a-z0-9._/-]*$/;
let currentPolicy: ProviderRequestTrafficPolicyProcessStateV1;
...
function clonePolicyValue(value: unknown, seen: Set<object>): unknown {
  if (
    value === null ||
    ["string", "number", "boolean", "bigint", "undefined"].includes(typeof value)
  ) {
    return value;
  }
  if (typeof value !== "object") {
    throw new Error("Provider request traffic policy TLS value is unsupported");
  }
  if (seen.has(value)) {
    throw new Error("Provider request traffic policy TLS value cannot be cyclic");
  }
  if (Array.isArray(value)) {
    seen.add(value);
    const cloned = value.map((entry) => clonePolicyValue(entry, seen));
    seen.delete(value);
  ...
  const cloned = Object.freeze(
    Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, clonePolicyValue(entry, seen)]),
    ),
  );
  seen.delete(value);
  return cloned;
...
function cloneRecord(
  ...
  const cloned = clonePolicyValue(value, new Set()) as Record<string, unknown>;
  if (cloned.rejectUnauthorized === false) {
...
export function compileProviderRequestTrafficPolicyV1(
...
export function registerProviderRequestTrafficPolicyForOwnerV1(
  ...
  const snapshot = compileProviderRequestTrafficPolicyV1(registration);
  ...
  const published = { owner: normalizedOwner, snapshot, active: true };
  ...
  return () => {
    ...
    if (currentPolicy === published) {
      currentPolicy = previous?.active ? previous : undefined;
    }
  };
}
...
export function snapshotProviderRequestTrafficPolicyProcessStateV1(): ProviderRequestTrafficPolicyProcessStateV1 {
  return currentPolicy;
}

export function restoreProviderRequestTrafficPolicyProcessStateV1(
  state: ProviderRequestTrafficPolicyProcessStateV1,
): void {
  currentPolicy = state;
}
...
export function evaluateCurrentProviderRequestTrafficPolicyV1(
  ...
  return currentPolicy
    ? evaluateProviderRequestTrafficPolicyV1(currentPolicy.snapshot, facts)
    : undefined;
}
