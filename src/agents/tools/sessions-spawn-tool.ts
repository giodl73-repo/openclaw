...
export function createSessionsSpawnTool(
  opts?: {
    ...
    /** Exact requester conversation instance used for managed regarding inheritance. */
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (
        requestedSkillName &&
        opts?.parentSkillInvocation &&
        !opts.parentSkillInvocation.executionHints?.usesSkills?.includes(requestedSkillName)
      ) {
        return jsonResult({
          status: "error",
          error: `Skill "${requestedSkillName}" is not declared by parent skill "${opts.parentSkillInvocation.skillName}".`,
        });
      }
      ...
      const result = await spawnSubagentDirect(
        ...
        {
          agentSessionKey: opts?.agentSessionKey,
          requesterSessionId: opts?.requesterSessionId,
          completionOwnerKey: opts?.completionOwnerKey,
          agentChannel: opts?.agentChannel,
          agentAccountId: opts?.agentAccountId,
          agentTo: opts?.agentTo,
