...
export type ModelProviderHostedBindingImplementationsV1 = {
  credentialSlot: {
    id: string;
    version: string;
...
export type PreparedHostedModelRequestV1 = {
  url: string;
  method: string;
  headers: Headers;
  body: Uint8Array;
...
export async function dispatchHostedModelRequestV1(params: {
  policy: ProviderRequestTrafficPolicySnapshotV1;
  providerId: string;
  providerLabel: string;
  endpointClass: ProviderEndpointClass;
  ...
  dispatcher: OneHopFetchDispatcher;
  request: PreparedHostedModelRequestV1;
  signal?: AbortSignal;
  createError: (code: HostedDispatchFailureCode, message: string) => Error;
}): Promise<{
  response: Response;
  ...
  const validateUrl = (url: URL) => {
    if (url.protocol !== "https:") {
      throw params.createError(
        "traffic-policy-route-mismatch",
        `${params.providerLabel} redirects must preserve HTTPS`,
      );
    }
  ...
  const result = await fetchWithSsrFGuard({
    ...
    init: {
      ...
      body: Buffer.from(params.request.body),
