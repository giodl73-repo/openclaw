    ...
    definitions: [
      {
        ...
        slotId: "lobster/substrate-token",
        placement: "header",
...
describe("Substrate LLM API hosted binding", () => {
    ...
    async (mode) => {
      ...
      const dispatch = vi.fn(async (request) => {
        ...
        if (mode === "local") {
          ...
          expect(request.credentialSlotRefs).toEqual(["lobster/substrate-token"]);
        }
          ...
          ? createLocalOneHopFetchDispatcher(
              async (_url, init) =>
                await dispatch({
                  ...
                  credentialSlotRefs: ["lobster/substrate-token"],
                }),
  ...
  it("fails closed on missing adapter or credential contributions before dispatch", () => {
    ...
    const incomplete = {
      ...preparedBundle,
      inventory: preparedBundle.inventory.filter(
        (entry) => entry.id !== "lobster/substrate-llmapi",
      ),
    };
