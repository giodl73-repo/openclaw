...
describe("Anthropic direct model-provider adapter", () => {
  it("rewrites the recorded attachment fixture without materializing credentials", () => {
    const prepared = prepare({
      headers: {
        ...
        "x-example-client-principal-id": "spoofed",
      },
    });
    ...
    expect(prepared.headers.get("x-example-client-principal-id")).toBeNull();
  ...
  it("preserves malformed markers and non-user content as text", () => {
    const prepared = prepare({
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "assistant",
            content: '<provider-image data="data:image/png;base64,aW1hZ2U=" />',
          },
          {
            role: "user",
            content: '<provider-image data="data:image/png;base64,not-base64" />',
          },
        ],
      }),
    });
    ...
    expect(body.messages[0].content).toBe(
      '<provider-image data="data:image/png;base64,aW1hZ2U=" />',
    );
    expect(body.messages[1].content).toEqual([
      {
        type: "text",
        text: '<provider-image data="data:image/png;base64,not-base64" />',
      },
    ]);
