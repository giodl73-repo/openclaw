...
export const ANTHROPIC_DIRECT_MODEL_ADAPTER_ID = "lobster/anthropic-direct" as const;
export const ANTHROPIC_API_KEY_SLOT_ID = "lobster/anthropic-key" as const;
...
const MARKER_PATTERN = /<(lobster-image|lobster-document)\b[^>]*?\bdata="(data:[^"]+)"[^>]*\/>/g;
...
function rewriteTextContent(value: string): unknown[] | undefined {
  ...
  for (const match of value.matchAll(MARKER_PATTERN)) {
    ...
    if (!parsed) {
      ...
      const type = match[1] === "lobster-document" ? "document" : "image";
      parts.push({
        type,
        source: {
          type: "base64",
