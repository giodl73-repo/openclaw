...
| `localization-catalogs`            | Report adopted English-source drift and block malformed current catalogs without credentials; manual/release runs enforce strict source/generated parity                                                              | Node-relevant changes                          |
...
3. `build-artifacts` and the locale checks overlap with the fast Linux lanes. The `localization-catalogs` job reports English-only drift and blocks malformed current catalogs for explicitly adopted areas; manual/release runs enforce strict parity, and its trusted refresh workflow opens an isolated generated PR with auto-merge disabled. Control UI and native app source PRs retain their existing isolated refresh workflows. Source CI blocks stale source inventories and unsafe localization calls. Generated PRs, manual CI, and release prep enforce full translated/platform-generated parity. Canonical `release/YYYY.M.PATCH` branches may include release-prep locale repairs with the other generated release output.
