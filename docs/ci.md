...
| `localization-catalogs`            | Fail closed on surface inventory for every current ready PR/main push; block catalog drift on ready same-repository default-base PRs, keep forks/other bases advisory, and enforce strict parity after refresh/manual runs | Every current ready PR, main push, and manual CI |
...
3. `build-artifacts` and the locale checks overlap with the fast Linux lanes. Drafts do not run `localization-catalogs`. Once ready, its surface inventory runs independently of Node change routing and fails closed for current targets; only proven historical manual targets may lack the script. The job fails a same-repository PR targeting the default branch when an adopted English source makes targets stale; forks and other bases remain advisory. A maintainer dispatch runs protected-`main` tooling against the exact PR head and commits one validated translation batch back under an exact-head lease. Fork/cross-repository merges and recovery runs use one generated follow-up PR with auto-merge disabled. Control UI and native app source PRs retain their conforming owner workflows. Generated changes, manual CI, and release prep enforce full translated/platform-generated parity.
