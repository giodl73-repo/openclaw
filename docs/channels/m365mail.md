...
Microsoft 365 Email connects a hosted OpenClaw agent to its provisioned mailbox. Lobster delivers inbound email activities to the Gateway. OpenClaw owns message parsing, sender policy, threading, Graph request bytes, and response handling; it does not acquire or store the Graph token.

This channel is for Lobster-hosted deployments. A standalone OpenClaw installation does not provision the mailbox, inbound activity route, or `lobster/graph-token` credential slot.

The owner-controlled hosted Graph binding is defined but inactive in this release slice. Lobster activation is a separate adoption step; until then, the live channel preserves its existing brokered egress path with the same owner-prepared request.

## Requirements

- A Lobster-provisioned Microsoft 365 agent identity and mailbox
...
- Lobster inbound activity delivery and brokered Graph egress configured for the container
- The Lobster forwarder configured with Gateway authentication for the webhook request

## Enable the channel

Lobster normally projects the required environment variables when it starts the container. Enable the channel in `openclaw.json`:
...
Do not configure a Graph bearer token in OpenClaw. When the hosted binding is activated, it resolves the exact `lobster/graph-token` credential slot and places it in the Authorization header after the owner has prepared the request.
...
Lobster can project these variables:
...
Replies to messages with a Graph message ID use the Graph reply endpoint. Messages without a usable message ID are sent as new mail with a normalized `Re:` subject.
...
Confirm that Lobster projected a non-empty `OPENCLAW_M365MAIL_AGENT_ID`. Do not substitute the recipient identity from an inbound activity.
...
Confirm that the hosted binding selected the `lobster/graph-token` slot, the configured Graph URL uses an allowed production origin, and the inbound recipient identity matches the configured mailbox identity.
