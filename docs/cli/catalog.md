...
| `openclaw catalog test-matrix` | List routed-operation test-plan candidates and any supplied coverage evidence.                                                                                                  | `--json`       |
...
| `catalog test-matrix --json` | 1              | `candidates`, `counts`, `generatedFrom`, `nodeCommandCandidates`, `schemaVersion`          |
...
The generated reference intentionally snapshots static descriptors, command routes, routed operations, catalog surfaces, and prompt projection metadata. `openclaw catalog list` fills `cli.runtimeCommands` from the current Commander tree at command execution time and `cli.pluginCommands` when `--plugin-descriptors` is requested. Node/gateway owners can supply approved `cli.nodeCommands` through the programmatic `buildCatalogList({ nodeCommands })` API; the root catalog CLI does not fetch live node inventory. Agents discover live node commands through the existing `nodes` tool with its `status` and `describe` actions.
...
| `cli.nodeCommands`    | Node/operator commands supplied by pairing, gateway, or node-host owners | Programmatic `buildCatalogList({ nodeCommands })`; live `nodes` tool `status`/`describe` |
...
| `session_status` | `agents`  | `low`    | `mixed`     | no           | `session_status` | `explicit-overlay`  |
...
| ----------------- | ------------------ | -------------------------- | -------- | ------------------------------------------------------------------------------------------------- |
| `agents-list`     | `routed-operation` | `openclaw agents`          | `low`    | `openclaw agents`, `openclaw agents list`                                                         |
| `channels-list`   | `routed-operation` | `openclaw channels list`   | `low`    | `openclaw channels list`                                                                          |
| `channels-status` | `routed-operation` | `openclaw channels status` | `low`    | `openclaw channels status`                                                                        |
| `config-get`      | `routed-operation` | `openclaw config get`      | `low`    | `openclaw config get`                                                                             |
| `config-unset`    | `routed-operation` | `openclaw config unset`    | `medium` | `openclaw config unset`                                                                           |
| `gateway-status`  | `routed-operation` | `openclaw gateway status`  | `low`    | `openclaw gateway status`                                                                         |
| `health`          | `routed-operation` | `openclaw health`          | `low`    | `openclaw health`                                                                                 |
| `models-list`     | `routed-operation` | `openclaw models list`     | `low`    | `openclaw models list`                                                                            |
| `models-status`   | `routed-operation` | `openclaw models status`   | `low`    | `openclaw models status`                                                                          |
| `plugins-list`    | `routed-operation` | `openclaw plugins list`    | `low`    | `openclaw plugins list`                                                                           |
| `sessions`        | `routed-operation` | `openclaw sessions`        | `low`    | `openclaw sessions`                                                                               |
| `status`          | `routed-operation` | `openclaw status`          | `low`    | `openclaw status`                                                                                 |
| `tasks-audit`     | `routed-operation` | `openclaw tasks audit`     | `low`    | `openclaw tasks audit`                                                                            |
| `tasks-list`      | `routed-operation` | `openclaw tasks list`      | `low`    | `openclaw tasks list`, `openclaw tasks`                                                           |
...
| `session_status`  | `tool`             | `session_status`           | `low`    | `session_status`, `session_status model=<provider/model>`                                         |
...
| `gateway`         | `command`          | `gateway`                  | `medium` | `gateway restart`, `gateway config.schema.lookup`, `gateway config.apply`                         |
