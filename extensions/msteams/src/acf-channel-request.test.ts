...
describe("Microsoft Teams ACF Channel request", () => {
  it("preserves the owner-prepared activity and selects the exact bearer slot", () => {
    ...
    expect(prepared.headers.has("cookie")).toBe(false);
    expect(prepared.headers.has("x-api-key")).toBe(false);
  });

  it("accepts normal Teams SDK sends that omit recipient from the activity body", () => {
    const activity = JSON.parse(fixture.request.body) as Record<string, unknown>;
    ...
    expect(new TextDecoder().decode(prepared.body)).toBe(JSON.stringify(activity));
  });

  it("accepts OpenClaw Teams SDK update bodies with their merged reference identity", () => {
    const activity = { type: "message", text: "updated" };
    const prepared = prepareMSTeamsAcfChannelRequestV1({
      ...fixture.request,
      method: "PUT",
