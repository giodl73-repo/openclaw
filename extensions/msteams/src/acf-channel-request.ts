import type { PreparedHostedProviderRequestV1 } from "../runtime-api.js";

export const MSTEAMS_ACF_CHANNEL_REQUEST_VERSION = "msteams-acf-channel-request/v1" as const;
export const MSTEAMS_ACF_BEARER_SLOT_ID = "lobster/acf-token" as const;
...
function resolveSlot(readiness: readonly MSTeamsAcfCredentialSlotReadinessV1[]) {
  ...
  if (
    ...
    throw new MSTeamsAcfRequestError(
      "credential-slot-incompatible",
      "Microsoft Teams ACF requires one exact lobster/acf-token credential slot",
    );
  }
  return slot;
}
...
function assertActivityIdentity(
  ...
  if (
    ...
    nestedId(activity.from) !== context.agentId ||
    nestedId(activity.recipient) !== context.recipientId ||
    nestedId(activity.conversation) !== context.conversationId
  ) {
    throw new MSTeamsAcfRequestError(
      "identity-mismatch",
...
export function prepareMSTeamsAcfChannelRequestV1(params: {
  ...
  for (const header of ["content-length", "host", "transfer-encoding"]) {
  ...
  if (method === "DELETE") {
    ...
    assertActivityIdentity(parseActivity(body), context);
