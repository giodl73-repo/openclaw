 ...
 * Lobster's per-user OpenClaw container in *brokered* mode: inbound email is
 * delivered by the Lobster runtime to a loopback webhook, and outbound replies
...
type M365MailConfigFields = {
  ...
  graphBaseUrl?: string;
  /** Loopback path the Lobster runtime posts inbound email activities to. */
  webhookPath?: string;
  dmPolicy?: "open" | "allowlist" | "disabled";
  /** Allowed sender email addresses (or AAD object IDs) for dmPolicy=allowlist. */
...
export interface ResolvedM365MailAccount {
   ...
   * `OPENCLAW_M365MAIL_AGENT_ID` env the Lobster container adapter projects from
...
// as a Bot Framework–derived "Activity". The Lobster runtime forwards that
