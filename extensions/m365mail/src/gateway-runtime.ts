...
import { normalizeM365MailWebhookPath, resolveAccount } from "./accounts.js";
...
export function registerM365MailWebhookRoute(params: {
  ...
  const routeKey = normalizeM365MailWebhookPath(account.webhookPath);
  ...
  if (activeRoute) {
    log?.info?.(`Deregistering stale route before re-registering: ${routeKey}`);
