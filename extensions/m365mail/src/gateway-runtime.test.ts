...
describe("collectM365MailGatewayStartupIssues", () => {
  it("reports a disabled account as an info-level issue and stops early", () => {
    ...
    const issue = first(issues, "disabled issue");
    expect(issue.code).toBe("disabled");
    expect(issue.logLevel).toBe("info");
  });
  ...
  it("flags an allowlist policy with no senders as a warn-level refusal", () => {
    ...
    expect(issues).toHaveLength(1);
    const issue = first(issues, "empty allowlist issue");
    expect(issue.code).toBe("empty-allowlist");
    expect(issue.logLevel).toBe("warn");
    expect(issue.message).toContain("refusing to start route");
  });
  ...
  it("refuses startup when the trusted mailbox identity is missing", () => {
    ...
    expect(issues).toHaveLength(1);
    const issue = first(issues, "missing identity issue");
    expect(issue.code).toBe("identity-missing");
    expect(issue.message).toContain("no trusted mailbox identity");
  });
...
describe("collectM365MailGatewayRoutingWarnings", () => {
  it("surfaces only the empty-allowlist issue as a formatted warning line", () => {
    ...
    const warning = first(warnings, "routing warning");
    expect(warning).toMatch(/^- Microsoft 365 Email: /);
    expect(warning).toContain("empty allowedSenders");
  });
...
describe("validateM365MailGatewayAccountStartup", () => {
  ...
  it("returns not-ok and warns for an empty allowlist", () => {
    ...
    const [warning] = first(log.warn.mock.calls, "warning log call");
    expect(warning).toContain("empty allowedSenders");
    expect(log.info).not.toHaveBeenCalled();
  });
...
describe("registerM365MailWebhookRoute idempotency", () => {
  ...
  it("registers the route on the account's webhook path with gateway auth", () => {
    ...
    expect(mockedRegister).toHaveBeenCalledTimes(1);
    const [arg] = first(mockedRegister.mock.calls, "route registration");
  ...
  it("rejects equivalent webhook paths with and without a leading slash", () => {
    mockedRegister.mockReturnValue(vi.fn());
    const dispose = registerM365MailWebhookRoute({
      account: makeAccount({ accountId: "acct-a", webhookPath: "/webhook/m365mail/shared" }),
      accountId: "acct-a",
    });

    expect(() =>
      registerM365MailWebhookRoute({
        account: makeAccount({ accountId: "acct-b", webhookPath: "webhook/m365mail/shared" }),
        accountId: "acct-b",
      }),
    ).toThrow(/already registered by account acct-a/);
    expect(mockedRegister).toHaveBeenCalledTimes(1);
