...
function commandPathLabel(commandPath: readonly string[]): string {
  return commandPath.join(" ");
}

function commandPathList(paths: readonly (readonly string[])[]): string {
  return paths.length > 0
    ? paths.map((commandPath) => code(commandPathLabel(commandPath))).join(", ")
    : "None";
}
      ...
      ...markdownTable(
        ...
        [
          ...
          [
            ...
            "List routed-operation test-plan candidates and any supplied coverage evidence.",
            code("--json"),
          ],
          [
      ...
      ...markdownTable(
        ...
        [
          ...
          [
            ...
            `test candidates ${testMatrix.counts.testCandidates}; routed operations with supplied evidence ${testMatrix.counts.evidencedRoutedOperations}`,
          ],
          [
            "Summary",
            code(summary.generatedFrom),
            `policy keys ${summary.attention.policyKeyIds.length}; confirmation-required surfaces ${summary.counts.confirmationRequiredSurfaces}`,
          ],
          [
            "Prompt projection",
      ...
      ...markdownTable(
        ...
        [
          ...
          [
            ...
            "Programmatic `buildCatalogList({ nodeCommands })`; live `nodes` tool `status`/`describe`",
          ],
        ],
      ),
      "",
